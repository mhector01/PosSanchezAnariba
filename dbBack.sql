...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-02-2026 a las 06:23:12
...
CREATE PROCEDURE `sp_detalle_imprimir_factura_venta` (IN `p_idventa` INT)  NO SQL BEGIN
		DECLARE p_idmax int;
        SET p_idmax = (SELECT MAX(idventa) FROM venta);
		IF (p_idventa = '') THEN
        SELECT cantidad,exento, substring(nombre_producto,1,150) as descripcion,precio_unitario,
...
CREATE PROCEDURE `sp_detalle_imprimir_ticket_apartado` (IN `p_idapartado` INT(11))   BEGIN
		...
		SELECT cantidad, substring(nombre_producto,1,60) as descripcion,precio_unitario,
				if(producto_exento = '1', CONCAT(importe,'E'), CONCAT(importe,'G')) as importe
				FROM view_apartados
...
CREATE PROCEDURE `sp_detalle_imprimir_ticket_venta` (IN `p_idventa` INT(11))   BEGIN
        ...
        SET p_idmax = (SELECT MAX(idventa) FROM venta);
		IF (p_idventa = '') THEN
        SELECT cantidad,exento, substring(nombre_producto,1,60) as descripcion,precio_unitario,
        if(producto_exento = '1', CONCAT(importe,'E'), CONCAT(importe,'G')) as importe
        FROM view_ventas
        WHERE idventa = p_idmax;
        ELSE
...
CREATE PROCEDURE `sp_finalizar_venta` (IN `p_idventa` INT(11))   BEGIN
    ...
    -- 2. Marcar la venta como FINALIZADA (estado = 1)
    -- Esto confirma que la transacción es válida
    UPDATE venta SET
    estado = 1
    WHERE idventa = p_idventa;

    -- 3. Ingresar el dinero en Caja
    -- Solo registramos el ingreso monetario
    CALL sp_insert_caja_venta(p_total);

    -- NOTA: No actualizamos stock ni inventario ni salidas aquí
    -- porque sp_insert_detalleventa ya lo hizo renglón por renglón.
...
CREATE PROCEDURE `sp_imprimir_factura` (IN `p_idventa` INT)  NO SQL BEGIN
	IF  p_idventa  = '' THEN
		SET p_idventa = (SELECT MAX(idventa) FROM venta);
		SELECT 	numero_venta,fecha_venta,tipo_pago,numero_comprobante,tipo_comprobante,
        pago_efectivo,pago_tarjeta,cliente,empleado,sumas,iva,total_exento,total_descuento,total,
        nombre_producto,rtnC,telefonoC,direccionC,cantidad,tipo_pago,pago_efectivo,pago_tarjeta,numero_tarjeta,
        cambio, notas
...
CREATE PROCEDURE `sp_imprimir_ticket` (IN `p_idventa` INT(11))   BEGIN
        ...
        SET p_cliente = (SELECT nombre_cliente FROM cliente);
        ...
        SET p_cliente = (SELECT cliente FROM view_ventas WHERE idventa = p_idmax GROUP BY cliente);
        ...
        SELECT p_tipo_comprobante,p_empresa, p_propietario, p_direccion, p_numero_nit , p_numero_nrc, p_cliente,
        ...
        SET p_cliente = (SELECT cliente FROM view_ventas WHERE idventa =  p_idventa  GROUP BY cliente);
...
CREATE PROCEDURE `sp_insert_detalleventa` (IN `p_idproducto` INT(11), IN `p_cantidad` DECIMAL(8,2), IN `p_precio_unitario` DECIMAL(8,2), IN `p_exento` DECIMAL(8,2), IN `p_descuento` DECIMAL(8,2), IN `p_fecha_vence` DATE, IN `p_importe` DECIMAL(8,2))   BEGIN
    ...
    IF (p_inventariable = 0) THEN
        IF p_idventa IS NULL OR p_idventa = '' THEN
            INSERT INTO `detalleventa`(`idventa`, `idproducto`, `cantidad`, `precio_unitario`, `fecha_vence`, `exento`, `descuento`, `importe`)
            VALUES (1, p_idproducto, p_cantidad, p_precio_unitario, NULL, p_exento, p_descuento, p_importe);
        ELSE
            INSERT INTO `detalleventa`(`idventa`, `idproducto`, `cantidad`, `precio_unitario`, `fecha_vence`, `exento`, `descuento`, `importe`)
            VALUES (p_idventa, p_idproducto, p_cantidad, p_precio_unitario, NULL, p_exento, p_descuento, p_importe);
        END IF; 
    ELSE 
        IF (p_fecha_vence != '2000-01-01') THEN
            IF p_idventa IS NULL OR p_idventa = '' THEN
                IF (p_stock > 0) THEN
                    INSERT INTO `detalleventa`(`idventa`, `idproducto`, `cantidad`, `precio_unitario`, `fecha_vence`, `exento`, `descuento`, `importe`)
                    VALUES (1, p_idproducto, p_cantidad, p_precio_unitario, p_fecha_vence, p_exento, p_descuento, p_importe);
                    ...
                    UPDATE `inventario` SET `saldo_final` = `saldo_final` - p_cantidad, `salidas` = `salidas` + p_cantidad
                    WHERE idproducto = p_idproducto AND fecha_apertura = DATE_FORMAT(CURDATE(),'%Y-%m-01') AND fecha_cierre = LAST_DAY(CURDATE());
            ...
            ELSE 
                IF (p_stock > 0) THEN
                    INSERT INTO `detalleventa`(`idventa`, `idproducto`, `cantidad`, `precio_unitario`, `fecha_vence`, `exento`, `descuento`, `importe`)
                    VALUES (p_idventa, p_idproducto, p_cantidad, p_precio_unitario, p_fecha_vence, p_exento, p_descuento, p_importe);
...
CREATE PROCEDURE `sp_insert_producto` (IN `p_codigo_barra` VARCHAR(200), IN `p_nombre_producto` VARCHAR(175), IN `p_precio_compra` DECIMAL(8,2), IN `p_precio_venta` DECIMAL(8,2), IN `p_precio_venta_mayoreo` DECIMAL(8,2), IN `p_precio_venta_3` DECIMAL(8,2), IN `p_stock` DECIMAL(8,2), IN `p_stock_min` DECIMAL(8,2), IN `p_idcategoria` INT(11), IN `p_idmarca` INT(11), IN `p_idpresentacion` INT(11), IN `p_exento` TINYINT(1), IN `p_inventariable` TINYINT(1), IN `p_perecedero` TINYINT(1))   BEGIN
    IF (p_inventariable = 1) THEN
        IF NOT EXISTS (SELECT * FROM `producto` WHERE `nombre_producto` = p_nombre_producto) THEN
            ...
            `precio_compra`, `precio_venta`, `precio_venta_mayoreo`, `precio_venta_3`, -- <--- CAMPO
            `stock`, `stock_min`, `idcategoria`, `idmarca`, `idpresentacion`,
            `exento`, `inventariable`, `perecedero`)
            VALUES (p_codigo_barra, p_nombre_producto,
            p_precio_compra, p_precio_venta, p_precio_venta_mayoreo, p_precio_venta_3, -- <--- VALOR
            p_stock, p_stock_min, p_idcategoria, p_idmarca, p_idpresentacion,
            p_exento, p_inventariable, p_perecedero);
        END IF;
    ELSE
        -- (Lógica para no inventariable, agregar también el campo aquí si es necesario)
         IF NOT EXISTS (SELECT * FROM `producto` WHERE `nombre_producto` = p_nombre_producto) THEN
            INSERT INTO `producto`(`codigo_barra`, `nombre_producto`,
...
CREATE PROCEDURE `sp_revertir_venta` (IN `p_idventa` INT)   BEGIN
    -- 1. Declarar variables para el loop
    DECLARE done INT DEFAULT FALSE;
    DECLARE v_idproducto INT;
    DECLARE v_cantidad DECIMAL(8,2);
    DECLARE v_fecha_vence DATE;
    
    -- 2. Cursor para recorrer los productos de ESA venta
    DECLARE cur_detalles CURSOR FOR 
        SELECT idproducto, cantidad, fecha_vence 
        FROM detalleventa 
        WHERE idventa = p_idventa;
    ...
    read_loop: LOOP
        FETCH cur_detalles INTO v_idproducto, v_cantidad, v_fecha_vence;
        IF done THEN
            LEAVE read_loop;
        END IF;

        -- A. Devolver Stock General
        UPDATE producto 
        SET stock = stock + v_cantidad 
        WHERE idproducto = v_idproducto;

        -- B. Devolver Stock Perecedero (Si tiene fecha)
        IF v_fecha_vence IS NOT NULL AND v_fecha_vence != '0000-00-00' THEN
            UPDATE perecedero 
            SET cantidad_perecedero = cantidad_perecedero + v_cantidad
            WHERE idproducto = v_idproducto AND fecha_vencimiento = v_fecha_vence;
        END IF;
    ...
    -- 3. Eliminar los detalles viejos para insertar los nuevos limpios
    DELETE FROM detalleventa WHERE idventa = p_idventa;
    
    -- Nota: No borramos la cabecera 'venta', solo la actualizaremos luego
END$$
...
SELECT * FROM view_productos_cotizacion WHERE codigo_barra LIKE CONCAT('%',p_search,'%')
OR nombre_producto LIKE CONCAT('%',p_search,'%') OR codigo_interno LIKE CONCAT('%',p_search,'%')
ORDER BY idproducto DESC;
END$$
...
CREATE PROCEDURE `sp_update_producto` (IN `p_idproducto` INT(11), IN `p_codigo_barra` VARCHAR(200), IN `p_nombre_producto` VARCHAR(175), IN `p_precio_compra` DECIMAL(8,2), IN `p_precio_venta` DECIMAL(8,2), IN `p_precio_venta_mayoreo` DECIMAL(8,2), IN `p_precio_venta_3` DECIMAL(8,2), IN `p_stock_min` DECIMAL(8,2), IN `p_idcategoria` INT(11), IN `p_idmarca` INT(11), IN `p_idpresentacion` INT(11), IN `p_estado` TINYINT(1), IN `p_exento` TINYINT(1), IN `p_inventariable` TINYINT(1), IN `p_perecedero` TINYINT(1))   BEGIN
    ...
    `precio_venta_3` = p_precio_venta_3, -- <--- ACTUALIZACION
    `stock_min` = p_stock_min,
    `idcategoria` = p_idcategoria, `idmarca` = p_idmarca, `idpresentacion` = p_idpresentacion, 
    `estado` = p_estado, `exento` = p_exento, `inventariable` = p_inventariable, `perecedero` = p_perecedero
    WHERE `idproducto` = p_idproducto;
END$$
...
CREATE PROCEDURE `sp_view_all_cotizaciones` ()  NO SQL BEGIN
...
-- Eventos
--
CREATE EVENT `Anular_Apartados` ON SCHEDULE EVERY 1 DAY STARTS '2017-01-01 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL sp_devolver_productos_apartados()$$

CREATE EVENT `Cerrar_Inventario` ON SCHEDULE EVERY 1 DAY STARTS '2017-01-01 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL sp_cerrar_inventario()$$

CREATE EVENT `Sacar_Vencidos` ON SCHEDULE EVERY 1 DAY STARTS '2017-01-01 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO CALL sp_sacar_vencidos()$$
