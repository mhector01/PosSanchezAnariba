...
const QuickCreateModal = ({ isOpen, title, label, value, onChange, onSave, onClose, loading }: any) => {
    ...
    return (
        <div className="absolute inset-0 z-[80] flex items-center justify-center bg-black/20 backdrop-blur-[2px] p-4 animate-in fade-in zoom-in duration-200">
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col border border-slate-200">
                ...
                <div className="flex-1 overflow-auto p-0 custom-scrollbar">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? ( <tr><td colSpan={5} className="p-10 text-center text-slate-400">Cargando historial...</td></tr> ) : history.length === 0 ? ( <tr><td colSpan={5} className="p-10 text-center text-slate-400">No hay ajustes registrados.</td></tr> ) : (
                                history.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50 transition">
                                        <td className="p-4 text-slate-500 font-mono text-xs">{new Date(item.fecha).toLocaleDateString()} {new Date(item.fecha).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</td>
                                        <td className="p-4"><div className="font-bold text-slate-700">{item.nombre_producto}</div><div className="text-[10px] text-slate-400 font-mono">{item.codigo_barra}</div></td>
                                        <td className="p-4 text-center"><span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${item.tipo === 'ENTRADA' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>{item.tipo === 'ENTRADA' ? 'Sobrante (+)' : 'Faltante (-)'}</span></td>
                                        <td className="p-4 text-center font-black text-slate-700">{item.cantidad}</td>
                                        <td className="p-4 text-slate-600 italic text-xs max-w-xs truncate">{item.motivo.replace('AJUSTE INVENTARIO: ', '')}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50 text-right"><button onClick={onClose} className="px-6 py-2 bg-white border border-slate-200 rounded-xl text-slate-600 font-bold hover:bg-slate-100 transition">Cerrar</button></div>
            </div>
        </div>
    );
};
...
export default function AdminProductsPage() {
  ...
  const initialForm: Product = {
    ...
    idcategoria: 0, idsubcategoria: 0, idpresentacion: 0, idmarca: 0, perecedero: 0, imagen: ''
  };
  ...
  const fetchCatalogs = async () => {
    try {
      const [resCat, resSub, resBrand, resPres] = await Promise.all([
        fetch('/api/categories'), fetch('/api/subcategories'), fetch('/api/brands'), fetch('/api/presentations')
      ]);
      setCategories(await resCat.json()); setSubcategories(await resSub.json());
      setBrands(await resBrand.json()); setPresentations(await resPres.json());
  ...
  useEffect(() => {
    ...
    } else { setFilteredSubcats([]); setForm(prev => ({ ...prev, idsubcategoria: 0 })); }
  ...
  const handleSaveQuick = async () => {
    ...
    } catch (e) { alert("Error de conexión"); } finally { setQuickModal(prev => ({ ...prev, loading: false })); }
  };
  ...
  const handleOpenAdjust = (prod: Product) => { setSelectedProduct(prod); setShowAdjustModal(true); };
  const handleSaveAdjust = async (idproducto: number, cantidad_sistema: number, cantidad_real: number, motivo: string) => {
      try {
          const res = await fetch('/api/inventory/adjust', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ idproducto, cantidad_sistema, cantidad_real, motivo, idusuario: user?.id }) });
          if(res.ok) { alert("Inventario ajustado"); setShowAdjustModal(false); fetchProducts(); } else { alert("Error al ajustar"); }
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        {isAdmin && (
            <div className="flex gap-2">
                <button onClick={() => setShowHistoryModal(true)} className="bg-slate-800 hover:bg-slate-900 text-white px-5 py-4 rounded-2xl font-bold shadow-lg shadow-slate-200 transition transform active:scale-95 flex items-center gap-2"><FileClock className="w-5 h-5" /> <span className="hidden md:inline">Historial</span></button>
                <button onClick={handleOpenCreate} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-4 rounded-2xl font-bold shadow-lg shadow-indigo-200 transition transform active:scale-95 flex items-center gap-2"><Plus className="w-5 h-5" /> Nuevo Producto</button>
            </div>
        )}
      </div>
      ...
      {showModal && isAdmin && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] relative">
            <QuickCreateModal isOpen={quickModal.isOpen} title="Nuevo Item" label="Nombre" value={quickModal.value} onChange={(val: string) => setQuickModal(p => ({...p, value: val}))} onSave={handleSaveQuick} onClose={() => setQuickModal({ type: '', isOpen: false, value: '', loading: false })} loading={quickModal.loading} />
                    ...
                    <div className="w-40 h-40 rounded-xl border-2 border-dashed border-slate-300 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 hover:bg-indigo-50 transition relative overflow-hidden group bg-white shadow-sm" onClick={() => fileInputRef.current?.click()}>
                        {form.imagen ? (
                            <>
                                ...
                                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition text-white font-bold text-xs">Cambiar</div>
                            </>
                        ) : (
                            <div className="text-center text-slate-400 p-4">
                                {uploading ? <RefreshCw className="w-8 h-8 animate-spin mx-auto"/> : <UploadCloud className="w-8 h-8 mx-auto mb-2"/>}
